...
class BombermanEnvironment:
    ...
    def step(self, action):
        ...
        # TODO: Check if the player killed the enemy using the bomb and update reward accordingly
        # Assuming you have a method that checks if enemy is killed
        # if self.enemy_killed_by_bomb():
        #     reward += 20
        ...
        while len(state_representation) < 143:  # 2 for player x,y and 10 for coins (5 coins x 2 coordinates each)
            state_representation.append(0) 
        # Check for game-ending conditions
        all_coins_collected = not bool(self.coin_positions)
        if not self.player.is_alive or all_coins_collected :
            self.game_over = True

        return state_representation, reward, self.game_over
