...
class BombermanEnvironment:
    ...
    # Initialize empty sets for walls and coins
        ...
        # Place crates
        crate_positions = set()
        num_crates = 10  # Number of crates to be placed
        while len(crate_positions) < num_crates:
            x = random.randint(0, (WIDTH // TILE_SIZE) - 1) * TILE_SIZE
            y = random.randint(0, (HEIGHT // TILE_SIZE) - 1) * TILE_SIZE
            if (x, y) not in walls and (x, y) not in coin_positions:
                crate_positions.add((x, y))
                coin_positions.add((x, y))  # Place a coin inside the crate
    ...
    def get_initial_grid_state(self):
        ...
        for x, y in self.crate_positions:
            state[self.pos_to_grid(x, y)] = 5  # 5 for crates
    ...
    def step(self, action): # reward policy 
        ...
        bomb_positions = set()
        if action == 4:
            bomb_positions.add((self.player.x, self.player.y))
            bomb_timer = 3
            ...
            if bomb_positions:
                ...
                if bomb_timer == 0:
                    ...
                    explosion_positions = [
                        ...
                        (self.player.x + TILE_SIZE, self.player.y),
                        (self.player.x - TILE_SIZE, self.player.y),
                        (self.player.x, self.player.y + TILE_SIZE),
                        (self.player.x, self.player.y - TILE_SIZE)
                    ]
                    for pos in explosion_positions:
                        if pos in self.crate_positions:
                            self.crate_positions.remove(pos)
                            # If there's a coin inside the crate, it remains in place
                        if pos == (self.player.x, self.player.y):
                            reward = -10  # Penalty if the player is caught in the explosion
                    bomb_positions = set()  # Reset bomb positions after explosion
