...
import numpy as np
import random
from agent_code.our_agent.My_code.bomber_environment import HEIGHT, WIDTH, TILE_SIZE
import pygame
...
class CoinCollectorAgent: # main logic behind the Deep Q-Learning agent.
    ...
    def act(self, state):
        ...
        return torch.argmax(q_values).item(), torch.argmax(q_values) #the agent chooses the action corresponding to the maximum Q-value.( single integer representing the chosen action.)

    def next_move(self, state):
        self.moves = ["UP", "DOWN", "LEFT", "RIGHT"]
        self.move = self.moves[self.act(state)]
